...
get_climate_data <- function(d.type, where, date) {
  tryCatch({
  ...
  tryCatch({
    ...
    if (nrow(stations.list) < 1) {
      ...
      dat <- data.frame()
      ## Define the number of rows in each smaller list
      rows.per.list <- 20
      
      ## Calculate the number of smaller lists needed
      num.lists <- ceiling(nrow(stations.list) / rows.per.list)
      
      #' Split the stations list into smaller lists with 20 stations at max.
...
in.f.dir <- 'D:/OneDrive/Work/Imaginative AI/Projects/WaterQualityApp/Data/WQ_Data_Nov2021.p'
...
# Create a lookup table for the parameter names
climate.parameters <- c("wind", "rain", "air.temp", "earth.temp", "soil.moist")
...
for (i in 1:nrow(filtered.metadata[1:2, ])) {
  ...
  for (date in sampeling.dates[1:2]) {
    ...
    for (data.type in climate.parameters[1:2]) {
      ...
      climate.dat <- get_climate_data(data.type, c(lat, lon, 100), 
